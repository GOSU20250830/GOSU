...
class OracleDB:
    ...
    async def check_tables(self):
        for k, v in TABLES.items():
            try:
                if k.lower() == "GOSU_graph":
                    await self.query(
                        "SELECT id FROM GRAPH_TABLE (GOSU_graph MATCH (a) COLUMNS (a.id)) fetch first row only"
                    )
                else:
...
N_T = {
    "full_docs": "GOSU_DOC_FULL",
    "text_chunks": "GOSU_DOC_CHUNKS",
    "chunks": "GOSU_DOC_CHUNKS",
    "entities": "GOSU_GRAPH_NODES",
    "relationships": "GOSU_GRAPH_EDGES",
}

TABLES = {
    "GOSU_DOC_FULL": {
        "ddl": """CREATE TABLE GOSU_DOC_FULL (
    ...
    "GOSU_DOC_CHUNKS": {
        "ddl": """CREATE TABLE GOSU_DOC_CHUNKS (
                    id varchar(256) PRIMARY KEY,
                    workspace varchar(1024),
    ...
    "GOSU_GRAPH_NODES": {
        "ddl": """CREATE TABLE GOSU_GRAPH_NODES (
    ...
    "GOSU_GRAPH_EDGES": {
        "ddl": """CREATE TABLE GOSU_GRAPH_EDGES (
                    id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    workspace varchar(1024),
    ...
    "GOSU_LLM_CACHE": {
        "ddl": """CREATE TABLE GOSU_LLM_CACHE (
                    id varchar(256) PRIMARY KEY,
                    send clob,
                    return clob,
                    model varchar(1024),
    ...
    "GOSU_GRAPH": {
        "ddl": """CREATE OR REPLACE PROPERTY GRAPH GOSU_graph
                VERTEX TABLES (
                    GOSU_graph_nodes KEY (id)
                        LABEL entity
                        PROPERTIES (id,workspace,name) -- ,entity_type,description,source_chunk_id)
                )
                EDGE TABLES (
                    GOSU_graph_edges KEY (id)
                        SOURCE KEY (source_name) REFERENCES GOSU_graph_nodes(name)
                        DESTINATION KEY (target_name) REFERENCES GOSU_graph_nodes(name)
                        LABEL  has_relation
...
SQL_TEMPLATES = {
    # SQL for KVStorage
    "get_by_id_full_docs": "select ID,NVL(content,'') as content from GOSU_DOC_FULL where workspace='{workspace}' and ID='{id}'",
    "get_by_id_text_chunks": "select ID,TOKENS,NVL(content,'') as content,CHUNK_ORDER_INDEX,FULL_DOC_ID from GOSU_DOC_CHUNKS where workspace='{workspace}' and ID='{id}'",
    "get_by_ids_full_docs": "select ID,NVL(content,'') as content from GOSU_DOC_FULL where workspace='{workspace}' and ID in ({ids})",
    "get_by_ids_text_chunks": "select ID,TOKENS,NVL(content,'') as content,CHUNK_ORDER_INDEX,FULL_DOC_ID  from GOSU_DOC_CHUNKS where workspace='{workspace}' and ID in ({ids})",
    "filter_keys": "select id from {table_name} where workspace='{workspace}' and id in ({ids})",
    "merge_doc_full": """ MERGE INTO GOSU_DOC_FULL a
                    USING DUAL
                    ON (a.id = '{check_id}')
                    WHEN NOT MATCHED THEN
                    INSERT(id,content,workspace) values(:1,:2,:3)
                    """,
    "merge_chunk": """MERGE INTO GOSU_DOC_CHUNKS a
                    USING DUAL
                    ON (a.id = '{check_id}')
                    WHEN NOT MATCHED THEN
    ...
    "entities": """SELECT name as entity_name FROM
        ...
        FROM GOSU_GRAPH_NODES WHERE workspace='{workspace}')
    ...
    "relationships": """SELECT source_name as src_id, target_name as tgt_id FROM
        ...
        FROM GOSU_GRAPH_EDGES WHERE workspace='{workspace}')
        WHERE distance>{better_than_threshold} ORDER BY distance ASC FETCH FIRST {top_k} ROWS ONLY""",
    "chunks": """SELECT id FROM
        (SELECT id,VECTOR_DISTANCE(content_vector,vector('[{embedding_string}]',{dimension},{dtype}),COSINE) as distance
        FROM GOSU_DOC_CHUNKS WHERE workspace='{workspace}')
        WHERE distance>{better_than_threshold} ORDER BY distance ASC FETCH FIRST {top_k} ROWS ONLY""",
    # SQL for GraphStorage
    "has_node": """SELECT * FROM GRAPH_TABLE (GOSU_graph
        MATCH (a)
        WHERE a.workspace='{workspace}' AND a.name='{node_id}'
        COLUMNS (a.name))""",
    "has_edge": """SELECT * FROM GRAPH_TABLE (GOSU_graph
        MATCH (a) -[e]-> (b)
    ...
    "node_degree": """SELECT count(1) as degree FROM GRAPH_TABLE (GOSU_graph
        MATCH (a)-[e]->(b)
    ...
    "get_node": """SELECT t1.name,t2.entity_type,t2.source_chunk_id as source_id,NVL(t2.description,'') AS description
        FROM GRAPH_TABLE (GOSU_graph
        MATCH (a)
        WHERE a.workspace='{workspace}' AND a.name='{node_id}'
        COLUMNS (a.name)
        ) t1 JOIN GOSU_GRAPH_NODES t2 on t1.name=t2.name
    ...
    "get_edge": """SELECT t1.source_id,t2.weight,t2.source_chunk_id as source_id,t2.keywords,
        ...
        ) t1 JOIN GOSU_GRAPH_EDGES t2 on t1.id=t2.id""",
    ...
    "merge_node": """MERGE INTO GOSU_GRAPH_NODES a
                    USING DUAL
                ...
                WHEN NOT MATCHED THEN
                    ...
                    values (:1,:2,:3,:4,:5,:6,:7) """,
    "merge_edge": """MERGE INTO GOSU_GRAPH_EDGES a
                    USING DUAL
